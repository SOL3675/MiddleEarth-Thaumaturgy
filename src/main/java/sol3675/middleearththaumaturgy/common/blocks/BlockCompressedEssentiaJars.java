...
public class BlockCompressedEssentiaJars extends BlockContainerAdvanced{
	
	public static BlockContainer instance;
	
	public BlockCompressedEssentiaJars() {
		setHardness(1F);
		setHarvestLevel("pickaxe", 0);
		setBlockName(LibMisc.MODID + ":" + "compressedjar");
	...
	public void onBlockPlacedBy(World w, int x, int y, int z, EntityLivingBase entity, ItemStack items){
		...
		if ((tile instanceof TileCompressedEssentiaJars)) {
			if (face == 0) {
				((TileCompressedEssentiaJars)tile).facing = 2;
			}
			if (face == 1) {
				((TileCompressedEssentiaJars)tile).facing = 5;
			}
			if (face == 2) {
				((TileCompressedEssentiaJars)tile).facing = 3;
			}
			if (face == 3) {
				((TileCompressedEssentiaJars)tile).facing = 4;
			}
		}
	}
	...
	public boolean onBlockActivated(World w, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)  {
		if (w.getTileEntity(x, y, z) instanceof TileCompressedEssentiaJars ) {
			TileCompressedEssentiaJars container = (TileCompressedEssentiaJars)w.getTileEntity(x, y, z);	  
	...
	public TileEntity createNewTileEntity(World w, int meta) {
		return new TileCompressedEssentiaJars();
	}
	...
	public void registerBlockIcons(IIconRegister icon) {
		iconJar = icon.registerIcon(LibMisc.MODFULLID + ":textures/models/" + "compressedJar");
		iconLiquid = icon.registerIcon(LibMisc.MODFULLID + ":" + "animatedglow");
		blockIcon = icon.registerIcon(LibMisc.MODFULLID + ":" + "compressedJar");
	}
