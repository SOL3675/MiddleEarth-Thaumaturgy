...
import sol3675.middleearththaumaturgy.common.tileentities.TileExtralargeEssentiaJar;
...
public class BlockExtralargeEssentiaJar extends BlockContainerAdvanced{
	
	public static BlockContainer instance;
	
	public BlockExtralargeEssentiaJar() {
		setHardness(1F);
		setHarvestLevel("pickaxe", 0);
		setBlockName(LibMisc.MODID + ":" + "extralargejar");
	...
	public void onBlockPlacedBy(World w, int x, int y, int z, EntityLivingBase entity, ItemStack items){
		...
		if ((tile instanceof TileExtralargeEssentiaJar)) {
			if (face == 0) {
				((TileExtralargeEssentiaJar)tile).facing = 2;
			}
			if (face == 1) {
				((TileExtralargeEssentiaJar)tile).facing = 5;
			}
			if (face == 2) {
				((TileExtralargeEssentiaJar)tile).facing = 3;
			}
			if (face == 3) {
				((TileExtralargeEssentiaJar)tile).facing = 4;
			}
		}
	}
	...
	public boolean onBlockActivated(World w, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)  {
		if (w.getTileEntity(x, y, z) instanceof TileExtralargeEssentiaJar ) {
			TileExtralargeEssentiaJar container = (TileExtralargeEssentiaJar)w.getTileEntity(x, y, z);	  
	...
	public TileEntity createNewTileEntity(World w, int meta) {
		return new TileExtralargeEssentiaJar();
	}
	...
	public void registerBlockIcons(IIconRegister icon) {
		...
		blockIcon = icon.registerIcon("middleearththaumaturgy" + ":" + "extralargeJar");
	}
